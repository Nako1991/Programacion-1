...
///Case Principal
void cazePrincipal (int colo, int Tablero[filas][columnas],stUnidad Equipo1[],stUnidad Equipo2[]);
...
///Case Ingreso Juego
void cazeIngresoJuego (int colo, char archivo[],int Tablero[filas][columnas],stUnidad Equipo1[],stUnidad Equipo2[]);
...
///Case Admin
void cazeAdmin (int colo, char archivo[]);
void verCuentas();
...
void cazeModificarCuenta(int colo, char archivo[], int id);
void mostrarCuentaExpecifica(char archivo[], int id, int supx, int supy);           //ARCHI muestra especifico.
int modificarRegistroExpecifico(char archivo[], int id, char nombre[], char pass[]);//ARCHI modifica especifico.
...
void cargarAtributos(stUnidad limite[], stUnidad ejercito[],int id, int supx, int supy, int limitipo,int i);
...
void animAtaque(int posx, int posy,int finx, int finy);
...
void creaEjercitosManual(stUnidad Equipo1[], stUnidad Equipo2[]);
...
int main()
{
    ...
    stUnidad  Equipo1  [DIM] ;
    stUnidad  Equipo2 [DIM] ;
   ...
   // dibujarVentanas(90,31,117,43);
    inicializarArchi();
    cazePrincipal (colo, Tablero, Equipo1,Equipo2);

    /// LINEA 1087 EL JUEGO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    //coordenadas();  //Mi mejor amigo.
...
int controlMenu(int supx, int supy, int opciones, int colorcito)
{
    ...
    do
    {
        ...
        case 72:
            ...
            break;
        case 80:
            if(y<supy+(opciones-1)*2)    //abajo   s
            {
                gotoxy(x,y);
                printf("  ");  //borro el cursor
                y=y+2;
...
void cazePrincipal (int colo,int Tablero[filas][columnas],stUnidad Equipo1[],stUnidad Equipo2[])
{
    ...
    do
    {
        ...
        case 5:
            ...
            cazeIngresoJuego (colo, archivoUsuarios,Tablero,Equipo1,Equipo2);
            Sleep(300);
            break;
        case 7:
            gotoxy(supx, supy+7);
...
void cazeIngresoJuego (int colo, char archivo[],int Tablero[filas][columnas],stUnidad Equipo1[],stUnidad Equipo2[])
{
    ...
    char pass[50]="", cuenta[50]="";
    char secret[30]= {"admin"}; //shh es secreto
    int cont=0,i=0; //cuenta los jugadores registrados para jugar
    stUsuario usuario;
                ...
                if (usuario.id>-1)
                {
                    if (usuario.ejercito[1].id==1)
                    {
                        ...
                        if(cont!=2)
                        {
                            ...
                            for(i=0; i<5; i++)
                            {
                                Equipo1[i]=usuario.ejercito[i];
                            }
                            */

                        }
                        //printf("se cargo el equipo %d",cont);
...
/*
                            for(i=0; i<5; i++)
                            {
                                Equipo2[i]=usuario.ejercito[i];
                            }
*/

                            system("cls");
                            creaTablero(Tablero);
                            creaEjercitosManual(Equipo1,Equipo2);


                            juego(Tablero,Equipo1,Equipo2);
                            ///!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!ACA ARRANCA EL FUNCION JUEGO!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            /// guarda cada usuario.ejercito en cada arreglo para arrancarlo
                        }
                    }
                    else
                    {
                        printf("olvidaste tu ejercito jug%i",cont);
                        limpiarCampo(supx,supy+1,cuenta);
                        limpiarCampo(supx,supy+3,pass);
                    }
...
void cazeCrearCuenta (int colo, char archivo[])
{
    int supx=96,supy=33,resp,exito=0;
    char pass[50]="", cuenta[50]="";
...
void cazeAdmin (int colo, char archivo[])  /// es importante para tener un control total del sistema
{
...
void cazeEditorCuentas(int colo, char archivo[])
{
    int supx=95,supy=4, busqueda=-1, resp;
    char cuenta[50]="";
    stUsuario usuario;
    dibujarVentanas(84,2,135,30);
    limpiarVentana(85,3,134,29);
    do
    {
        ...
        case 3:
            ...
            if (usuario.id>-1)
            {
                printf("se encontro usuario: id.%i",busqueda);
                limpiarCampo(supx,supy+1,cuenta);
                cazeModificarCuenta(colo, archivo, busqueda);
            }
            else
            {
...
void cazeModificarCuenta(int colo, char archivo[], int id)
{
    int supx=95,supy=19, resp, retorno;
    char cuenta[50]="", pass[50]="";
    do
    {
        dibujoMenuModificarCuenta(supx, supy);
        mostrarCuentaExpecifica(archivo, id, supx, supy);
        ...
        case 5: //modificar
            ...
            limpiarCampo(supx,supy+1,cuenta);
            limpiarCampo(supx,supy+3,pass);
            retorno=modificarRegistroExpecifico(archivo, id, cuenta, pass);
            gotoxy(supx, supy+5);
            if(retorno==0)
                printf("Modificado");
            else if(retorno==-1)
...
void mostrarCuentaExpecifica(char archivo[], int id, int supx, int supy)
{
    stUsuario auxUsuario;
...
int modificarRegistroExpecifico(char archivo[], int id, char nombre[], char pass[])
{
    stUsuario auxUsuario;
    int resp=0;
    FILE * archi=NULL;
    ...
    if (archi!=NULL)
    {
        ...
        fseek(archi, sizeof(stUsuario)*(id-1), SEEK_SET);
        //le asigno lo que cargue a una estruc que sera subida.
        if ((strlen(nombre))>3)
        {
...
void cazeEditorEjercitos(int colo, char archivo[], stUsuario usuario)
{
    ...
    int supx=95,supy=4, resp, dimension=10;
    stUnidad limite[dimension];
    ...
    do
    {
        ...
        case 1:  //por nombre
            ...
            Sleep(500);
            limpiarVentana(85,3,134,29);
            crearTipo (limite,usuario.ejercito, dimension, 95, 4);
        ...
        case 5:
            gotoxy(supx, supy+5);
            printf("En construccion disculpe");
            //////algoooo;
...
int crearTipo (stUnidad limite[], stUnidad ejercito[], int dimension, int supx, int supy)
{
    ...
    do
    {
        ...
        gotoxy(supx,supy+1);
        printf("1.Heroe 2.Tanque 3.Soldado");
        gotoxy(supx,supy+2);
        hidecursor(1);
        scanf("%i",&tipo);
        hidecursor(0);
...
///elige color 1azul 2verde 3celeste 4rojo 5violeta 6amarillo 7blanco 8gris 9turqueza
...
void avanzaUnidad(int tablero[filas][columnas],stUnidad  Ejercito[],int UnidadActiva)
{
    ...
    do
    {
        fflush(stdin);
        control=getch();
        // barraEstado("Se ingreso correctamente a la funcion avanza unidad", "esperando ingreso switch");
...
//falta agrega que seleccione en pantalla la unidad enemiga

        {
            if (idEnemigo!=-1)
                ...
                {
                    animAtaque(EquipoATK[idActivo].posx,EquipoATK[idActivo].posy,EquipoDEF[idEnemigo].posx,EquipoDEF[idEnemigo].posy);
